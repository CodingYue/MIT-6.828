...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, UPAGES, 
			ROUNDUP(npages * sizeof(struct PageInfo), PGSIZE), PADDR(pages), PTE_P | PTE_U);
	...
	boot_map_region(kern_pgdir, UENVS, 
			ROUNDUP(NENV * sizeof(struct Env), PGSIZE), PADDR(envs), PTE_P | PTE_U);
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("pp_ref != 0 or pp_link is not NULL");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pte_t *pte = ((pte_t *)page2kva(page)) + PTX(va);
	return pte;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (size_t size = 0; size < len; ) {
		void *dst = (void*) va + size;
		pte_t *pte = pgdir_walk(env->env_pgdir, dst, 0);
		if ((*pte & perm) != perm) {
			user_mem_check_addr = (uint32_t) dst;
			return -E_FAULT;
		}
		#define min(a, b) ((a) < (b) ? (a) : (b))
		size += (ROUNDUP(dst+1, PGSIZE) - dst);
	}
